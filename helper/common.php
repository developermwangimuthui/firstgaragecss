...
class wms_core {
	...
	*/
	public function get_all_model_list($con) {
		$model = array();
		$result = mysql_query("SELECT *,ma.make_name FROM tbl_model mo INNER JOIN tbl_make ma ON mo.make_id = ma.make_id order by ma.make_name ASC",$con);
		while($row = mysql_fetch_array($result)){
			$model[] = array(
				'model_id'		=> $row['model_id'],
				'make_id'		=> $row['make_id'],
	...
	*/
	public function getSellPartsMonthlyData($con, $year) {
		...
		$result = mysql_query("select *,sum(total_parts) as total_parts from vw_parts_sold where year_name = '".$year."' group by month_name");
		while($row = mysql_fetch_assoc($result)){
			$data[] = $row;
		}
		return $data;
	...
	public function sendCustomerEmailNotification($con, $car_id, $progress, $invoice_id) {
		if($data = $this->getCustomerAndCarDetailsByCarId($con, $car_id)) {
			...
			if($site_config_data = $this->getWebsiteSettingsInformation($con)) {
				$from = $site_config_data['email'];
				$to = $data['c_email'];
				$subject = 'Repair Car Progress Notification';
				$headers = "From: " . strip_tags($from) . "\r\n";
				$headers .= "Reply-To: ". strip_tags($from) . "\r\n";
				$headers .= "MIME-Version: 1.0\r\n";
				$headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
	...
	public function saveUpdateMechanicsInformation($con, $data, $image_url) {
		if(!empty($data)) {
			...
			if($data['mechanics_id'] == '0') {
				mysql_query("INSERT INTO tbl_add_mechanics(m_name,m_cost,m_phone_number,m_password,m_email,m_present_address,m_permanent_address,m_notes,m_image,designation_id,status,joining_date) values('$data[txtSName]','$data[cost_per_month]','$data[txtPhonenumber]','$data[txtSPassword]','$data[txtSEmail]','$data[present_address]','$data[permanent_address]','$data[notes]','$image_url','$data[designation]','$data[status]','$join_date')", $con);
			} else {
				mysql_query("UPDATE `tbl_add_mechanics` SET `m_name`='".$data['txtSName']."',`m_cost`='".$data['cost_per_month']."',`m_phone_number`='".$data['txtPhonenumber']."',`m_password`='".$data['txtSPassword']."',`m_email`='".$data['txtSEmail']."',`m_present_address`='".$data['present_address']."',`m_permanent_address`='".$data['permanent_address']."',`m_notes`='".$data['notes']."',`m_image`='".$image_url."',designation_id='".$data['designation']."',status='".$data['status']."',joining_date='".$join_date."' WHERE mechanics_id='".(int)$data['mechanics_id']."'", $con);
			}
		}
	}
	...
	public function saveUpdateCustomerInformation($con, $data, $image_url) {
		if(!empty($data)) {
			if($data['customer_id'] == '0') {
				mysql_query("INSERT INTO tbl_add_customer(c_name,c_email, c_address, c_home_tel,c_work_tel,c_mobile,c_password,image) values('$data[txtCName]','$data[txtCEmail]','$data[txtCAddress]','$data[txtCHomeTel]','$data[txtCWorkTel]','$data[txtCMobile]','$data[txtCPassword]','$image_url')", $con);
			} else {
				mysql_query("UPDATE `tbl_add_customer` SET `c_name`='".$data['txtCName']."',`c_email`='".$data['txtCEmail']."',`c_address`='".$data['txtCAddress']."',`c_home_tel`='".$data['txtCHomeTel']."',`c_work_tel`='".$data['txtCWorkTel']."',`c_mobile`='".$data['txtCMobile']."',`c_password`='".$data['txtCPassword']."',`image`='".$image_url."' WHERE customer_id='".(int)$data['customer_id']."'", $con);
			}
		}
	}
	...
	public function saveUpdateRepairCarInformation($con, $data, $image_url) {
		if(!empty($data)) {
			if($data['repair_car'] == '0') {
				mysql_query("INSERT INTO tbl_add_car(repair_car_id,car_name,customer_id,car_make,car_model,year,chasis_no,car_reg_no,VIN,note,added_date,image) values('$data[hfInvoiceId]','$data[car_names]','$data[ddlCustomerList]','$data[ddlMake]','$data[ddlModel]','$data[ddlYear]','$data[car_chasis_no]','$data[registration]','$data[vin]','$data[car_note]','$data[add_date]','$image_url')", $con);
			} else {
				mysql_query("UPDATE `tbl_add_car` SET `customer_id`='".$data['ddlCustomerList']."',`car_name`='".$data['car_names']."',`car_make`='".$data['ddlMake']."',`car_model`='".$data['ddlModel']."',`year`='".$data['ddlYear']."',`chasis_no`='".$data['car_chasis_no']."',`car_reg_no`='".$data['registration']."',`VIN`='".$data['vin']."',`note`='".$data['car_note']."',`added_date`='".$data['add_date']."',`image`='".$image_url."' WHERE car_id='".$data['repair_car']."'", $con);
			}
		}
	}
	...
	*/
	public function getAllMechanicsList($con) {
		$data = array();
		$result = mysql_query("SELECT *,d.title FROM tbl_add_mechanics m LEFT JOIN tbl_mechanics_designation d ON d.designation_id = m.designation_id ORDER BY m.mechanics_id DESC", $con);
		while($row = mysql_fetch_assoc($result)){
			$data[] = $row;
		}
		return $data;
