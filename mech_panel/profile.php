...
if(isset($_POST['txtUserName'])){
	...
	$wms->updateMechanicesUserProfile($link, $_POST, $image_url);
...
if(!empty($_SESSION['objMech']['image'])){
	$image = WEB_URL.'img/employee/'.$_SESSION['objMech']['image'];
	$img_track = $_SESSION['objMech']['image'];
}
  ...
  <div align="right" style="margin-bottom:1%;"> <button  onclick="$('#from_admin_profile').submit();" class="btn btn-success" type="submit" data-toggle="tooltip" href="javascript:;" data-original-title="Update Profile"><i class="fa fa-save"></i></button></div>
    ...
    <div class="box box-success">
      <div class="box-header">
        <h3 class="box-title"><i class="fa fa-edit"></i> Update User Profile</h3>
		<br/><br/><h5 class="label label-danger"><i class="fa fa-gear"></i> After change your profile system will logout automatically.</h5>
      </div>
      <form onsubmit="return validateMe();" id="from_admin_profile" method="post" enctype="multipart/form-data">
        <div class="box-body">
          <div class="form-group">
            <label for="txtUserName"><span style="color:red;">*</span> Name:</label>
            <input type="text" name="txtUserName" value="<?php echo !empty($_SESSION['objMech']['name']) ? $_SESSION['objMech']['name'] : ''; ?>" id="txtUserName" class="form-control" required />
          </div>
          <div class="form-group">
            <label for="txtEmail"><span style="color:red;">*</span>Email :</label>
            <input type="text" name="txtEmail" value="<?php echo !empty($_SESSION['objMech']['email']) ? $_SESSION['objMech']['email'] : ''; ?>" id="txtEmail" class="form-control" required />
          </div>
          <div class="form-group">
            <label for="txtPassword"><span style="color:red;">*</span>Password :</label>
            <input type="text" name="txtPassword" value="<?php echo !empty($_SESSION['objMech']['password']) ? $_SESSION['objMech']['password'] : ''; ?>" id="txtPassword" class="form-control" required />
          </div>
          <div class="form-group">
		...
		<input type="hidden" name="hdnUserId" value="<?php echo !empty($_SESSION['objMech']['user_id']) ? $_SESSION['objMech']['user_id'] : '0'; ?>">
      </form>
      <!-- /.box-body -->
    </div>
    <!-- /.box -->
